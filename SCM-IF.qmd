...
Harrow_plt <- function(Data) {
  ggplot()+
  geom_line(data = Data |> filter(Model == "dndc"), mapping = aes(date, Water, col = "dndc")) +
...
ggplot() +
  geom_line(data = STC |> filter(Model == "dndc"), mapping = aes(date, temp, colour = "dndc")) +
  geom_point(data = STC |> filter(Model != "dndc"), mapping = aes(date, temp, colour = "obs"), size = 3) +
  scale_color_manual(breaks=c('dndc', 'obs'),
                     values=c('dndc'='red4', 'obs'='blue4'))+
  theme_base(base_family = "times") +
  theme(legend.position = "top",
        ...
        axis.title = element_text(face = "bold", size = 20)) +
  labs(color = NULL, y = expression(bold(Soil~Temperature ^{0}~C)))
